...
public class Tester extends Photo
{
	...
	public static void main(String[] args)
	{
		...
		ShelfLife s = new ShelfLife();
		GrabPhoto gs = new GrabPhoto();
        Thread th = new Thread(gs);
        th.start();
        gs.run();
        
		//s.GenerateRecipeIngredients();
		String[] ingredients = new String[9];
		
		//System.out.println(Photo.fromFridge().toString());
		...
		keyboard.close();*/
		Set set = s.GenerateRecipeIngredients().entrySet();
		System.out.println(s.GenerateRecipeIngredients().toString());
		...
		for(int i = 0; iter.hasNext(); i++)
		{
			...
			{
			Map.Entry mpe = (Map.Entry)iter.next();
			ingredients[i] = mpe.getValue().toString();
			}
		}
		...
		new URL("http://api.yummly.com/v1/api/recipes?_app_id=ff685a45&_app_key=3ee0e0125e7c0acdc06aa2c4c1f91758&"
				//+ "q=" 
				//+ "meat" 
				//+ "+" 
				//+ "loaf"  
				+ "&allowedIngredient[]=" 
				+ s.getFoodItem1().toString()
				+ "&allowedIngredient[]=" 
				+ s.getFoodItem2().toString()
				+ "&allowedIngredient[]=" 
				+ s.getFoodItem3().toString());
